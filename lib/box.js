...
class Box {
    ...
    /**
     * This function name was a typo. Just leaving it here for 1 release so people have time to change it out.
     *
     * @deprecated
     * @param {String} serviceName
     * @param {FactoryInterface|Function|Object} serviceFactory
    ...
    get(name) {
        ...
        if (name.includes('.')) {
            deepObjectProperty = name.split('.').slice(1).join('.');
            name = name.split('.')[0]; // eslint-disable-line no-param-reassign
        }
        ...
        if (_.isUndefined(instance) || !factory.isSingleton()) {
            ...
            if (_.isFunction(factory) && factoryMethod.length !== dependencies.length) {
        ...
        if (!!deepObjectProperty) {
            return _.get(instance, deepObjectProperty);
        }
